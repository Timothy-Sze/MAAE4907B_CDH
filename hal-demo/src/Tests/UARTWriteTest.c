...
void taskUARTWriteTest(void *arguments) {
...
Boolean UARTWriteTest() {
	...
	xTaskGenericCreate(taskUARTWriteTest, (const signed char*)"UARTtest-0", 1024, (void*)&UARTtestBus[0], 2, &UART0testHandle, NULL, NULL);
	xTaskGenericCreate(taskUARTWriteTest, (const signed char*)"UARTtest-2", 1024, (void*)&UARTtestBus[1], 2, &UART2testHandle, NULL, NULL);
